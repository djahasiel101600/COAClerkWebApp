...
def postdataendpoint(request, fund):
    ...
    if request.method == "POST":
        ...
        app_model = [model for model in apps.get_app_config('rci').get_models() if re.match(fund, model.__name__)]
        if len(app_model) == 1:
...
def listView(request):
    ...
    for model in app_models:
        queryset = model.objects.values('check_no', 'dte', 'payee', 'nature_of_transaction','grossAmount')
    ...
    if combined_queryset:
        ...
        return render(request, 'rci/components/table.html', {'data':page_obj})

    return HttpResponse("Queryset is empty")